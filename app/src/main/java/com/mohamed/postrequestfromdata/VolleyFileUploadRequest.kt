...
open class VolleyFileUploadRequest(
   ...
   // private val boundary = "imageRequest${System.currentTimeMillis()}"
    ...
    private fun processParams(dataOutputStream: DataOutputStream, params: MutableMap<String?, String?>, encoding: String) {
        try {
            params.forEach {
    ...
    private fun processData(dataOutputStream: DataOutputStream, data: Map<String, FileDataPart>) {
        data.forEach {
            ...
            var bufferSize = Integer.min(bytesAvailable, maxBufferSize)
            val buffer = ByteArray(bufferSize)
            var bytesRead = fileInputStream.read(buffer, 0, bufferSize)
            while (bytesRead > 0) {
                dataOutputStream.write(buffer, 0, bufferSize)
                bytesAvailable = fileInputStream.available()
                bufferSize = Integer.min(bytesAvailable, maxBufferSize)
